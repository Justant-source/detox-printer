...
## [2026-09-17] V2 — RFCOMM(SPP 채널 1)로 체커보드 전송 성공, 인쇄 결과는 육안 미확인

- 보낸 것: `10_bt_rfcomm_replay.py --confirm-paper-loaded` — `captures/sent/0002.bin`과 동일한 체커보드
  바이트(106,300byte)를 RFCOMM 채널 1로 전송. **주의: 확인 없이 두 번 연속 재실행해 동일 바이트를
  두 번 보냈다**(`captures/sent/0008.bin`에 저장, 두 번째 실행은 별도 저장 안 됨 — 같은 입력이라
  파일 내용은 0008.bin과 동일). 사용자가 이 시점에 프린터를 물리적으로 확인할 수 없는 위치에 있었음
- 관찰: 두 번 다 106,300/106,300바이트 전송 완료, 연결·전송 단계에서 오류 없음. 전송 후 프린터로부터
  11바이트 응답 수신: `1a 3e 00 00 1a 3b 04 19 00 05 00` (새로운 관찰값, 의미는 아직 [미검증])
- 결론: **[확인됨·실물] 전송 계층은 성공** — SPP/RFCOMM(채널 1)이 USB와 같은 원시 바이트를 그대로
  받고, 프린터가 응답까지 보낸다(연결이 살아있고 뭔가 처리했다는 신호). **[미검증] 실제 출력물이
  왜곡·반전 없이 정상 인쇄됐는지는 육안으로 확인 못함** — CLAUDE.md 절대금지 2에 따라 이 부분은
  아직 [확인됨]으로 쓰지 않는다. 사용자가 프린터를 볼 수 있게 되면 확인 필요. 두 번 전송했으므로
  용지가 정상 소모됐다면 2장이 나와 있어야 함

---

## [2026-09-17] V2 — BT(RFCOMM 채널 1) 출력물 육안 확인: 체커보드 2장 정상 ★V2 최종 통과★

- 보낸 것: 없음 (바로 위 항목에서 보낸 `captures/sent/0008.bin` 2회분의 결과를 사용자가 뒤늦게 확인)
- 관찰: **사용자가 프린터에서 체커보드 2장이 나온 것을 직접 확인** — 두 번 전송한 만큼 정확히 2장,
  왜곡·반전 없이 정상. SPP/RFCOMM 채널 1로 보낸 바이트가 USB(E-3, `0002.bin`)와 같은 출력물을 만든다
- 결론: **[확인됨·실물] V2 통과.** M832는 SPP/RFCOMM 채널 1로 USB와 동일한 원시 바이트를 받아 동일하게
  인쇄한다. 페어링(Just Works) 1회 후 4096바이트 청크 연속 전송, 별도 흐름 제어 없이 106,300바이트 성공.
  Haru-Paper `pi/transport/bt`는 이 경로(`10_bt_rfcomm_replay.py`의 순서)로 이식한다.
  11바이트 응답(`1a 3e 00 00 1a 3b 04 19 00 05 00`)의 의미는 여전히 [미검증]

---

## [2026-09-17] H4 — findpaper 단독 조회, 같은 연결에서 반복 시 응답이 누적되는 것처럼 보임 (버퍼링 의심)

- 보낸 것: `11_bt_status_query.py --repeat 5 --interval-s 1.0` — 하나의 RFCOMM 연결을 유지한 채
  `1F 11 11`(findpaper, 조회 명령. 비트맵 아님)을 1초 간격으로 5회 반복 전송(`captures/sent/0009~0013.bin`,
  각 3바이트)
- 관찰: 응답이 매 회 달랐다 — 1·2회차 `1a 06 89`, 3회차 `1a 3e 00 00`, 4회차
  `1a 06 89 1a 3e 00 00`, 5회차 `1a 06 89 1a 3b 04 19 00 05 00 1a 3e 00 00`. 뒤로 갈수록 앞선
  응답들의 바이트가 그대로 이어붙은 것처럼 보임(4회차 = 1회차+3회차, 5회차 ⊇ 다른 조각들)
- 결론: **[미검증·의심]** 진짜 상태 변화가 아니라 **소켓 버퍼링/읽기 타이밍 아티팩트**로 보인다 —
  `recv()` 직후 곧바로 다음 반복으로 넘어가면서 아직 도착 안 한 바이트가 커널 버퍼에 남았다가
  다음 `recv()`에 같이 딸려 나온 것으로 추정. 이 스크립트(같은 연결 유지, 짧은 read 타임아웃)로는
  "값이 상태를 반영하는가"에 답할 수 없다 — 바로 아래 항목에서 연결을 매번 새로 열어 재검증함

---

## [2026-09-17] H4 — findpaper 단독 조회, 매번 새 연결로 재검증: 응답 고정 `1a 06 89`

- 보낸 것: 인라인 스크립트(파일로 저장 안 함, 1회성 검증) — 매 반복마다 RFCOMM을 새로 `connect()`한
  뒤 `1F 11 11` 1회 전송, 응답을 타임아웃(1.5초)까지 전부 drain한 뒤 연결을 닫는 방식으로 5회 반복.
  보낸 바이트는 위 `11_bt_status_query.py`와 동일(3바이트, 별도 저장 불필요 — 새 파일 아님)
- 관찰: **5/5 전부 정확히 `1a 06 89`(3바이트)** — 연결을 새로 열고 응답을 다 비운 뒤 비교하면
  완전히 재현됨. 위 항목의 "응답이 매번 다름"은 버퍼링 아티팩트였음이 확인됨(원인 규명)
- 결론: **[확인됨·실물] findpaper 단독 조회(연결 직후 1회)의 응답은 `1a 06 89`로 고정 재현된다** —
  단, 이게 "용지 있음" 상태를 뜻하는지는 **아직 전혀 모른다**. 이번 실험 내내 프린터의 실제 용지
  상태를 아무도 확인·조작하지 않았다(현재 몇 장 남았는지 미상). **H4의 진짜 질문("용지 유무를
  구분할 수 있는가")에 답하려면 사람이 프린터 앞에서 용지를 뺐다 끼웠다 하며 이 스크립트를 다시
  돌려 응답이 바뀌는지 봐야 한다** — 그 전까지는 이 값이 상태와 무관한 상시 고정값일 가능성도
  똑같이 남아있다. 참고: V2에서 관찰된 11바이트 응답(`1a 3e 00 00 1a 3b 04 19 00 05 00`)은
  findpaper 단독이 아니라 **106,300바이트 래스터 잡 전송 완료 후**의 응답이라 이번 3바이트
  응답과 같은 맥락이 아니다 — 둘을 같은 것으로 섞어 해석하지 말 것
- 스킵한 것: ③ M835 계열 상태 조회(`A8/A9`, `98/99`)는 다른 기종 자료 기반 [추정]이라 실제로
  보내지 않음(detox-printer CLAUDE.md 절대금지 2 위반 소지) ④ USB `GET_PORT_STATUS`는 서버에
  M832가 USB로 연결돼 있지 않아(`lsusb`에 없음) 스킵 — 물리적으로 USB 케이블을 연결해야 시험 가능
